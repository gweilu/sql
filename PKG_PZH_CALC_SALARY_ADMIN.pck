create or replace package PKG_PZH_CALC_SALARY_ADMIN is
  ...
  PROCEDURE P_HR_ADMIN_QINGLIANG(V_STARTDATE DATE,V_ENDDATE DATE,V_RECDATE DATE,V_MONTH   VARCHAR2,V_EMPIDS  VARCHAR2,V_SSGID   VARCHAR2,V_ITEMVALUE VARCHAR2,V_ITEMKEY VARCHAR2);
  PROCEDURE P_HR_ADMIN_XILI(V_STARTDATE DATE,V_ENDDATE DATE,V_RECDATE DATE,V_MONTH   VARCHAR2,V_EMPIDS  VARCHAR2,V_SSGID   VARCHAR2,V_ITEMVALUE VARCHAR2,V_ITEMKEY VARCHAR2);
...
connect by prior o.orgid=o.parentorgid) then 
  ...
  --管理人员洗礼费用计算
   --洗理
  PROCEDURE P_HR0_ADMIN_XILI(V_STARTDATE DATE, --
                              V_ENDDATE   DATE, --统计结束日期
                              V_RECDATE   DATE, --记录日期
                              V_MONTH     VARCHAR2, --统计月份（没有用到，为统一参数接口而留）
                              V_EMPIDS    VARCHAR2, --人员编号
    ...
    *****************************************************************************/
    P_MONTH VARCHAR2(20):= REPLACE(V_MONTH,'-');
    V_COUNT number := 0;
    V_Washing_Cost_Men number :=10;
    V_Washing_Cost_Women number :=12;
  ...
  BEGIN
    ...
    select COUNT(1) into v_count
    from hr_salary_configs c
    where c.section='WASHING_COST_MEN';
    if v_count>0 then 
      select value into V_Washing_Cost_Men
      from hr_salary_configs c
      where c.section='WASHING_CONST_MEN';
      end if; 
    --获取女员工标准
    select COUNT(1) into v_count
    from hr_salary_configs c
    where c.section='WASHING_COST_WOMEN';
    if v_count>0 then 
      select value into V_Washing_Cost_Women
      from hr_salary_configs c
      where c.section='WASHING_COST_WOMEN';
      end if; 
    --1有人 追加或者重新生成部分人员
      ...
      INSERT INTO HRMONTHLYRECGD
        (SEQID, SALARYDATE, RECDATE, EMPID, REAL, ITEMVALUE, ITEMKEY)
          SELECT S_HRMONTHLYRECGD.NEXTVAL,
                     ...
                     SYSDATE,
                     S.EMPID,
                     case when e.sex='1' then V_Washing_Cost_Men else V_Washing_Cost_Women end,
                     v_itemvalue,
                     v_itemkey               
  ...
  -----------------------------------------------------------------------------
  --清凉费
  PROCEDURE P_HR_ADMIN_QINGLIANG(V_STARTDATE DATE, --
                              V_ENDDATE   DATE, --统计结束日期
                              V_RECDATE   DATE, --记录日期
                              V_MONTH     VARCHAR2, --统计月份（没有用到，为统一参数接口而留）
                              V_EMPIDS    VARCHAR2, --人员编号
  ...
  BEGIN  
    ...
    IF V_COUNT>0 THEN  
     ...
     --普通管理人员标准
       SELECT COUNT(1) INTO V_COUNT
           FROM HR_SALARY_CONFIGS C
     WHERE C.SECTION = 'COOL_WAGE_STD2';   
    IF V_COUNT>0 THEN  
        SELECT VALUE
          INTO V_COOL_WAGE_STD2
          FROM HR_SALARY_CONFIGS C
         WHERE C.SECTION = 'COOL_WAGE_STD2';
     END IF;
     --获取本月的天数
      ...
      INSERT INTO HRMONTHLYRECGD
        (SEQID, SALARYDATE, RECDATE, EMPID, REAL, ITEMVALUE, ITEMKEY)
          SELECT S_HRMONTHLYRECGD.NEXTVAL,
                     ...
                     SYSDATE,
                     S.EMPID,
                     case when e.empptype in ('11',15) then ROUND(V_COOL_WAGE_STD1*V_DAYS,2)
                       else ROUND(V_COOL_WAGE_STD2*V_DAYS,2) end,
                     v_itemvalue,
                     v_itemkey               
          FROM mcemployeeinfogs         e,
               HR_STAFFSALARYSETSETUPGD S
