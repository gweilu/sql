...
select rb.orgname 单位,
       ...
       round((select sum(r.realbczb) from pzhdrb r where r.routeid=rb.routeid and r.recorddate between trunc(r.recorddate,'mm') and rb.recorddate),1) 累计班次正班,
       round((select sum(r.realbcrq) from pzhdrb r where r.routeid=rb.routeid and r.recorddate between trunc(r.recorddate,'mm') and rb.recorddate),1) 累计班次日勤,
       round((select sum(r.realbczb) from pzhdrb r where r.routeid=rb.routeid and r.recorddate between trunc(r.recorddate,'mm') and rb.recorddate),2) +round((select sum(r.realbcrq) from pzhdrb r where r.routeid=rb.routeid and r.recorddate between trunc(r.recorddate,'mm') and rb.recorddate),2) 累计班次合计,
       rb.mails 日公里,
       (select sum(r.mails) from pzhdrb r where r.routeid=rb.routeid and r.recorddate between trunc(r.recorddate,'mm') and rb.recorddate) 累计公里,
       ys.kpje 当日客票,
       ys.dhjje 当日IC卡,
